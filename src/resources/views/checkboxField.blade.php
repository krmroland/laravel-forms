<div class="form-group">
  @include('forms::label')
   ...
   <label class="custom-control custom-checkbox {{ $field->class }}">
   @if ($field->hasKeys)
     <input  name="{{ $field->name }}[]" type="checkbox" class="custom-control-input 
     {{ $errors->has($field->name)?'is-invalid':'' }}" value="{{ $key }}" 
     {{ $field->checkedStatus($key)}}
     >
     @else
     <input  name="{{ $field->name }}[]" type="{{ $field->type }}" class="custom-control-input 
